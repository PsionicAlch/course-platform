...
func AddChapter(dbFacade SqlDbFacade, id, title string, chapter int, content, fileChecksum, fileKey, courseKey string) error {
	query := `INSERT INTO course_chapters (id, title, chapter, content, course_id, file_checksum, file_key) VALUES (?, ?, ?, ?, (SELECT id FROM courses WHERE file_key = ?), ?, ?);`
...
func UpdateChapter(dbFacade SqlDbFacade, id, title string, chapter int, content, fileChecksum, fileKey, courseKey string) error {
	query := `UPDATE course_chapters SET title = ?, chapter = ?, content = ?, course_id = (SELECT id FROM courses WHERE file_key = ?), file_checksum = ?, file_key = ? WHERE id = ?;`
