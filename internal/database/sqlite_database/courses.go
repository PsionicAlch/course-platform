...
func (db *SQLiteDatabase) GetAllCoursesPaginated(page, elements int) ([]*models.CourseModel, error) {
	query := `SELECT id, title, slug, description, thumbnail_url, banner_url, content, published, author_id, file_checksum, file_key, created_at, updated_at FROM courses ORDER BY updated_at DESC, title ASC LIMIT ? OFFSET ?;`
...
func (db *SQLiteDatabase) SearchCoursesPaginated(term string, page, elements int) ([]*models.CourseModel, error) {
	query := `SELECT id, title, slug, description, thumbnail_url, banner_url, content, published, author_id, file_checksum, file_key, created_at, updated_at FROM courses WHERE LOWER(title) LIKE '%' || ? || '%' OR LOWER(description) LIKE '%' || ? || '%' ORDER BY updated_at DESC, title ASC LIMIT ? OFFSET ?;`
